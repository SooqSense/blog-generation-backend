"""
Chat service for database operations with user isolation
"""
import logging
...
class ChatService:
    """Service for managing chat data with user isolation"""
    ...
    def get_user_by_id(self, user_id: int) -> Optional[User]:
        ...
        if not DJANGO_AVAILABLE:
            self.logger.warning("Django not available - cannot get user")
            return None
    ...
    def create_chat_session(self, user_id: int, session_id: str, title: str = None) -> Optional[ChatSession]:
        ...
        if not DJANGO_AVAILABLE:
            self.logger.warning("Django not available - cannot create chat session")
            return None
