...
class BlogWriter:
    ...
    def __init__(self, use_custom_llm=False, topic=None, keywords=None, tone="professional", 
        ...
        if use_custom_llm:
            ...
            # Use GPT-4o for better long-form content generation and higher temperature for creativity
            self.llm = ChatOpenAI(model="gpt-4o", temperature=0.8, max_tokens=4000)
...
This style replication is MANDATORY and should be your PRIMARY focus alongside word count requirements."""
    ...
    def image_prompt_generator(self):
        return Agent(
            role="Expert Visual Content Strategist & Image Prompt Specialist",
            goal=f"Analyze blog content structure and generate compelling, detailed image prompts that perfectly complement the written content about {self.topic}.",
            backstory="You are a creative visual content strategist with expertise in translating written content into powerful visual concepts. You have extensive experience in creating image prompts that enhance reader engagement and comprehension. You understand how to identify key sections that would benefit from visual representation and craft detailed, specific prompts that result in high-quality, relevant images.",
            verbose=True,
            llm=self.llm
        )
...
REMEMBER: You are the FINAL GUARDIAN. This blog post MUST be exactly {self.length_min}-{self.length_max} words and MUST be the most comprehensive resource on {self.topic} ever created. FAILURE IS NOT AN OPTION. COUNT WORDS AND EXPAND AGGRESSIVELY IF NEEDED."""
    ...
    def image_prompt_generation_task(self):
        description = f"""🎨🎨🎨 CRITICAL IMAGE PROMPT GENERATION MISSION 🎨🎨🎨

You are analyzing the completed blog post about: {self.topic}

⚠️ ABSOLUTE REQUIREMENTS ⚠️
- Generate EXACTLY {self.max_image_prompts} high-quality image prompts
- Each prompt must be detailed, specific, and visually compelling
- Prompts should complement different sections of the blog content
- Focus on key headings, concepts, and important sections

🎯 IMAGE PROMPT GENERATION STRATEGY 🎯

STEP 1: CONTENT ANALYSIS
- Analyze the blog structure and identify key sections
- Look for main headings (##) and important concepts
- Identify sections that would benefit most from visual representation
- Consider the overall topic and target audience

STEP 2: PROMPT CREATION REQUIREMENTS
For each image prompt, you MUST include:
✅ Specific visual style (e.g., "professional infographic", "modern illustration", "realistic photo")
✅ Key elements to include in the image
✅ Color scheme suggestions when relevant
✅ Composition and layout guidance
✅ Context related to the blog section it represents
...
1. HEADER/HERO IMAGES: Main topic visualization
2. SECTION ILLUSTRATIONS: Specific concept explanations
3. INFOGRAPHICS: Data or process visualization
4. CONCEPTUAL IMAGES: Abstract ideas made visual
5. COMPARISON CHARTS: Side-by-side comparisons
6. WORKFLOW DIAGRAMS: Step-by-step processes

📋 EXAMPLE PROMPT FORMAT 📋
"Create a professional infographic showing [specific concept] with [visual elements]. Use a modern, clean design with [color scheme]. Include [specific data/elements] arranged in [layout description]. Style should be [professional/creative/technical] to match the blog's tone about {self.topic}."

🚨 CRITICAL REQUIREMENTS 🚨
- Generate EXACTLY {self.max_image_prompts} prompts
- Each prompt must be unique and serve a different purpose
- Prompts should cover different sections of the blog
- Ensure prompts are actionable for image generation
- Match the tone and professionalism of the blog content

⚠️ FINAL CHECKLIST ⚠️
Before submitting, verify:
✅ Exactly {self.max_image_prompts} prompts generated
✅ Each prompt is detailed and specific
✅ Prompts cover different blog sections
✅ Visual styles are appropriate for the topic
✅ All prompts are ready for AI image generation
✅ Prompts enhance the blog content effectively

REMEMBER: These image prompts will be used to create visual content that enhances reader engagement and comprehension of {self.topic}. Quality and relevance are paramount."""

        expected_output = f"🎨 MANDATORY: Exactly {self.max_image_prompts} detailed, high-quality image prompts. Each prompt must be 50-150 words, highly specific, visually descriptive, and directly relevant to different sections of the blog content. Prompts should be ready for immediate use with AI image generation tools and should enhance the overall blog experience."
